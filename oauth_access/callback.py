...
class Callback:
    ...
    def __init__(self, request, access, token):
        ...
        if not request.user.is_authenticated():
            ...
            authenticated = True
            user = request.user
        self.redirect_to = self.redirect_url(request)
        if user:
            kwargs = {}
            #if not authenticated: #TODO GUardar el identificador para usuarios ya existentes

...
class MultiFileField(Field):
    ...
    def clean(self, data):
        ...
        try:
            ...
            raise ValidationError(ugettext(u"No se ha enviado ninguna foto, comprueba que son del formato y tamaño permitidas."))
        except KeyError:
            raise ValidationError(ugettext(u"No se ha enviado ninguna foto."))
        
        if self.strict and len(f) != self.count:
            raise ValidationError(ugettext(u"Se han envíado un número de fotos no permitidas."))
